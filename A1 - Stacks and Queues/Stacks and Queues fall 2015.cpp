...
/*
	Nathan Borup
	CS 2420
	Stacks and Queues - Assignment 1
*/

template <typename T>
class CustomStack {
public:
	CustomStack(const unsigned int initCapacity);
	~CustomStack();
...
template <typename T>
CustomStack<T>::CustomStack(const unsigned int capacity){
	arr = new T[capacity];
	currentSize = 0;
...
template <typename T>
void CustomQueue<T>::push_back(const T& item){
	if (currentSize == capacity)
	{
		throw 1;
	}
